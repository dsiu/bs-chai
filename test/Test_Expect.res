...
describe("Expect", () => {
  ...
  describe("Combos.End", () => {
    ...
    it("to_deep_equal", () =>
      expect(list{{"a": 1}, {"a": 2}}) |> to_deep_equal(list{{"a": 1}, {"a": 2}})
    )
    it("to_be", () => expect(list{{"a": 1}, {"a": 2}}) |> to_be(list{{"a": 1}, {"a": 2}}))
    it("to_be_a", () => expect("foo") |> to_be_a("string"))
    it("to_be_ok", () => expect(true) |> to_be_ok)
    it("to_include", () => expect([1, 2, 3]) |> to_include(3))
    it("to_equal", () => expect(123) |> to_equal(123))
    it("to_eql", () => expect({"a": 1, "b": 2}) |> to_eql({"a": 1, "b": 2}))
    it("to_be_above", () => expect(2) |> to_be_above(1.))
    it("to_be_below", () => expect(1) |> to_be_below(2.))
    it("to_be_at_least", () => expect(1) |> to_be_at_least(0.))
    it("to_be_at_most", () => expect(1) |> to_be_at_most(2.))
    it("to_be_within", () => expect(1) |> to_be_within(0., 2.))
    it("to_be_instanceof", () => expect([1, 2, 3]) |> to_be_instanceof(_Array))
    it("to_have_property", () => expect({"a": 1}) |> to_have_property("a", ~value=1))
    ...
    it("to_be_one_of", () => expect(1) |> to_be_one_of([2, 3, 1]))
    it("to_have_keys", () => expect({"a": 1, "b": 2, "c": 3}) |> to_have_keys(["a", "b", "c"]))
